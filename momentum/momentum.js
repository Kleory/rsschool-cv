...
function getImage() {
  ...
  setTimeout(function() { btnBgr.disabled = false }, 1000);
...
function showDate() {
  let today = new Date(),
    ...
    cDate.innerHTML = `${addDayOfWeek(week)}, ${addMonth(month)} ${addZero(day)}, ${year}`;
}
function addMonth(m) {
  return monthArr[m];
}
...
nameUser.addEventListener('click', () => {nameUser.textContent = ''})
